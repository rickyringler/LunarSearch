...
void performSearch(const std::string& luaQuery) {
    ...
    // Load the documents into Lua as a table
    lua_newtable(L);
    ...
    if (lua_istable(L, -1)) {
        ...
        std::cerr << "Invalid result from Lua query." << std::endl;
    }
...
int main() {
    // Read Lua query from file
    std::ifstream file("query.lua");
    if (!file.is_open()) {
        std::cerr << "Error opening query.lua" << std::endl;
        return 1;
    }
